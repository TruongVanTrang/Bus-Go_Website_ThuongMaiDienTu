...
const updateFile = (filePath) => {
  ...
  // Find precisely using substring
  const startStr = '{/* Hình ảnh (nếu có) */}';
  const startIdx = content.indexOf(startStr);
  if (startIdx === -1) {
    console.log('Start str not found in', filePath);
    return;
  }

  // Find the exact end of the block
  // We know it ends with `)}` shortly after `</Camera className="h-4 w-4" /> Hình ảnh đính kèm`
  let tempIdx = content.indexOf('Hình ảnh đính kèm', startIdx);
  let endIdx = content.indexOf(')}', tempIdx);

  if (endIdx === -1) {
    console.log('End str not found in', filePath);
    return;
  }
  ...
  content = content.substring(0, startIdx) + replaceStr + content.substring(endIdx + 2);
